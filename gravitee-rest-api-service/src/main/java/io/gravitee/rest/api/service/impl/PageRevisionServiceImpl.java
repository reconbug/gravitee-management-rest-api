...
public class PageRevisionServiceImpl extends TransactionalService implements PageRevisionService {
    ...
    private PageRevision convert(Page page) {
        ...
        revision.setRevision(findLastByPageId(page.getId())
                ...
                .orElse(1));
        revision.setReferenceId(page.getReferenceId());
        revision.setReferenceType(page.getReferenceType());

        // contact pageId with revision as identifier
        String revId = String.format("%s:%03d", revision.getPageId(), revision.getRevision() );
    ...
    private String canonicalRevision(PageRevision page) {
        ...
        builder.append('\n');
        builder.append(Optional.ofNullable(page.getReferenceId()).map(c -> c.trim()).orElse(""));
        builder.append('\n');
        builder.append(Optional.ofNullable(page.getReferenceType()).map(c -> c.name().trim()).orElse(""));
        builder.append('\n');
